...
Im Hypergraphen hat jeder Knoten und jede Kante ein Symbol als Beschriftung.
--> Es gibt drei Arten:
    
    TSymbol --> behriftet eine Hyperkante = ein Atom (T = Terminal = echtes Atom, kein Platzhalter)

    BondSymbol --> beschriftet eine Knoten = eine Bindung (Knoten im Hypergraph sind Bindungen!)

    NTSymbol --> beschrifet einr Hyperkante = Platzhalter / noch nicht expandiert (NT = Non-Terminal, wird erst in grammar.py gebraucht)
'''
...
class NTSymbol(object):
    ...
    Verständlicher:
        Beschriftet eine Hyperkante, die noch NICHT einem echten Atom entspricht. Es ist ein Platzhalter für einen teil-Graphen ist der noch expandiert wereden muss.
    '''
    ...
    #Sortiert nach bond_type damit die Reihenfolge keine Rolle spielt beim Vergleichen 
    ...
    @property
    def symbol(self):
        return f'R' #R für Rest? -> Platzhalter 
...
class BondSymbol(object):
    ...
    Verständlich:
        bond_type aka Bindungstypen sind:
            
            1 = Einfachbindung (C-C)
            2 = Doppelbinung (C=C)
            3 = Dreifachbindung (C#C)
            12 = Aromatisch (Benzol) -->
    '''
